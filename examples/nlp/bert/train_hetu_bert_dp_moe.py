...
import os
import hetu as ht
from hetu_bert_moe import BertForPreTraining
...
def pretrain(args):
    ...
    # Define Bert-MoE Model, 12B parameters: 24 Layer, 64 Experts per Layer.
    ...
    file_dir = '/jizhicfs/pinxuezhao/Hetu/examples/nlp/bert/data/hdf5_lower_case_1_seq_len_512_max_pred_80_masked_lm_prob_0.15_random_seed_12345_dupe_factor_5/%s/'%dataset
    ...
    model = BertForPreTraining(config=config, device_id=args.gpu_id)
    ...
    _,_, masked_lm_loss, next_sentence_loss, moe_loss = model(input_ids, token_type_ids, attention_mask, masked_lm_labels, next_sentence_label)
    ...
    if args.comm_mode is None:
        if args.gpu < 0:
            ...
            ctx = ht.gpu(args.gpu)
        executor = ht.Executor(eval_nodes, dynamic_memory=True, ctx=ctx)
    else:
        strategy = ht.dist.DataParallel(args.comm_mode)
        executor = ht.Executor(eval_nodes, dist_strategy=strategy, dynamic_memory=True)
    ...
    for ep in range(num_epochs):
        ...
        for train_file in train_files:
            ...
            for i in range(batch_num_per_device):
                ...
                # end_time = time.time()
                if device_id == 0 and global_step_num % 2 == 0 and global_step_num != 0:
                    end_time = time.time()
                    print('HETU-MoE: [Epoch %d] (Iteration %d): MLM_loss = %.3f, NSP_loss = %.6f, Throughput = %.3f samples/s'%(ep,step_num, masked_lm_loss_mean_out, next_sentence_loss_mean_out, args.train_batch_size * nrank * 2.0/(end_time-start_time)))
                    start_time = time.time()
                step_num += 1
                global_step_num += 1
